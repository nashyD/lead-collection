// One-time seeder: pull Gaston County car dealerships from the Google Places
// API (New) and insert them into the Supabase `dealerships` table. Exact mirror
// of seed-complexes.mjs, just a different search term and target table.
//
// Run locally (NOT deployed). Re-running is safe: it only INSERTS dealerships it
...
//   node scripts/seed-dealerships.mjs
//
...
async function searchCity(city) {
  ...
  for (let page = 0; page < MAX_PAGES_PER_CITY; page++) {
    ...
    const body = { textQuery: `car dealerships in ${city}, North Carolina` };
...
async function main() {
  ...
  console.log(`\nUpserting ${rows.length} dealerships (existing ones left untouched)…`);
  const resp = await fetch(`${SUPABASE_URL}/rest/v1/dealerships?on_conflict=place_id`, {
    method: 'POST',
    headers: {
      apikey: SUPABASE_KEY,
  ...
  console.log(`Done. ${inserted.length} new dealerships inserted.`);
  console.log('Sample:');
  for (const c of inserted.slice(0, 8)) console.log(`  • ${c.name} — ${c.address}`);
  console.log('\nReview the list in the dashboard → Dealerships tab and delete any non-dealership results.');
}
